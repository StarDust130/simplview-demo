...
const metrics: Metric[] = [
  { value: "3 Days", label: "saved per report request", detail: "answers in 30 seconds" },
  { value: "$20K+", label: "saved vs Traditional BI setup costs", detail: "setup costs avoided" },
  { value: "$80K+", label: "in analyst costs automated per year", detail: "lean teams move faster" },
  { value: "10+", label: "industry verticals", detail: "D2C, BFSI, manufacturing & more" },
];
...
function LiveChart({ heights, className = "" }: { heights: number[], className?: string }) {
  return (
    <div className={`flex items-end gap-2 w-full h-full ${className}`}>
      {heights.map((h, i) => (
        <motion.span
          ...
          animate={{
            height: [`${h}%`, `${Math.max(20, h - 15)}%`, `${Math.min(100, h + 15)}%`, `${h}%`]
          }}
          transition={{
...
function MiniSignal({ label, dark = false, isWide = false }: { label: string; dark?: boolean; isWide?: boolean }) {
  const isChart = label.includes("Chart") || label.includes("Graph") || label.includes("Geometric");

  if (isChart) {
    return (
      <div className={`mt-5 flex items-end gap-2 rounded-xl border p-4 ${
        dark ? "border-white/10 bg-white/5" : "border-slate-200/50 bg-slate-50/50"
  ...
  return (
    <div className={`mt-5 flex items-center justify-between rounded-xl border px-5 py-4 ${
      dark ? "border-white/10 bg-white/5 text-white" : "border-slate-200/50 bg-slate-50/50 text-slate-700"
    }`}>
      <span className="text-xs font-black uppercase tracking-widest">{label}</span>
      <Cloud className={dark ? "h-5 w-5 text-cyan-400" : "h-5 w-5 text-blue-600"} />
    </div>
  );
}
...
export default function StatsSection() {
  ...
  return (
    <section id="problems" ref={ref} className="relative z-10 overflow-hidden bg-[#F3F1EA] py-24 text-slate-950">
      <div className="absolute inset-x-0 top-0 h-[30rem] bg-[url('/bg.png')] bg-cover bg-center opacity-10 pointer-events-none" />
      ...
      <div className="relative mx-auto max-w-7xl px-6 lg:px-8">
        ...
        >
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {metrics.map((metric, index) => (
              <div key={metric.value} className="relative overflow-hidden rounded-xl bg-white/5 p-6 text-white border border-white/5">
        ...
        <div className="mt-24 grid gap-8 lg:grid-cols-[1fr_1fr] lg:items-end">
          <motion.div initial={{ x: -24, opacity: 0 }} whileInView={{ x: 0, opacity: 1 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
            ...
            <h2 className="mt-6 max-w-2xl text-4xl font-black leading-[1.05] tracking-tight md:text-5xl lg:text-6xl text-slate-900">
              ...
              <span className="font-serif italic font-medium text-slate-600">team faces</span>{" "}
              every week
            </h2>
          </motion.div>

          <motion.p
            initial={{ x: 24, opacity: 0 }} whileInView={{ x: 0, opacity: 1 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.1 }}
            ...
            return (
              <motion.article
                ...
                className={`group relative flex flex-col overflow-hidden rounded-3xl border p-7 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-xl cursor-default ${
                  isHero ? "md:col-span-1 lg:col-span-1 lg:row-span-2" :
                  isWide ? "md:col-span-2 lg:col-span-3" :
                  "md:col-span-1 lg:col-span-1"
                } ${
                  isHero ? "bg-[#0A0D14] text-white border-slate-800" :
                  isBlue ? "bg-blue-600 text-white border-blue-500" :
                  isDark ? "bg-[#0A0D14] text-white border-slate-800" :
                  "border-slate-200/60 bg-white text-slate-900"
                }`}
              >
                <div className="relative flex items-start justify-between gap-4">
                  <div className={`grid h-12 w-12 shrink-0 place-items-center rounded-xl shadow-md transition-transform duration-300 group-hover:scale-105 ${
                    isDark || isBlue ? "bg-white text-slate-950" : "bg-slate-900 text-white"
                  }`}>
                ...
                <div className={`relative flex flex-1 flex-col ${isWide ? "lg:flex-row lg:items-center lg:justify-between lg:gap-12" : ""}`}>
                  
                  <div className={`mt-6 ${isWide ? "lg:max-w-md lg:mt-4" : ""}`}>
                    <h3 className={`${isHero ? "text-3xl lg:text-4xl" : "text-xl"} font-black leading-tight tracking-tight`}>
                      {problem.title}
                    </h3>
                    <p className={`mt-4 text-[14px] font-medium leading-relaxed ${
                  ...
                  <div className={`flex flex-col flex-1 ${isWide ? "lg:w-1/2 lg:flex-none" : "mt-auto pt-6"}`}>
                    ...
                    {!isHero && (
                      <div className="mt-auto">
                        <MiniSignal label={problem.visual} dark={isDark || isBlue} isWide={isWide} />
                      </div>
                    )}

                    <p className={`border-t pt-5 text-sm font-semibold italic ${
                      isWide ? "mt-6" : "mt-6"
                    } ${
                      isDark || isBlue ? "border-white/10 text-white/50" : "border-slate-100 text-slate-400"
                    }`}>
                      &quot;{problem.quote}&quot;
                    </p>
                  </div>
